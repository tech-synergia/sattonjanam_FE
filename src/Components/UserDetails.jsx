  ...
  const fetchProfile = async () => {
    ...
    try {
      const response = await axios.get(
        `https://sattonjanam-be.vercel.app/api/v1/auth/currentUser`,
        {
          headers: {
    ...
    try {
      if (window.confirm(`Are you sure to delete an image?`)) {
        ...
        const res = await axios.post(
          `https://sattonjanam-be.vercel.app/api/v1/image/destroy`,
          { public_id: images },
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      const res = await axios
        .patch(
          `https://sattonjanam-be.vercel.app/api/v1/user/update/${profileData._id}`,
          newUser,
          (profileData.image = images.url),
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: token,
      ...
      <Form className="popUpUserForm">
        ...
        <Form.Item label="Partner Education" htmlFor="partnerEductation">
          <Input
            ...
            value={profileData.partnerEductation}
            onChange={readValue}
          />
        </Form.Item>
