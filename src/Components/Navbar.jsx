...
function Navbar() {
  ...
  const adminRoute = () => {
    return (
      <ul className="navbar-nav">
        ...
        <NavLink to={`/`} onClick={logoutUser} className="dropdown-item">
          <button>Logout</button>
        </NavLink>
        <NavLink to={`/enquireform`} className="nav-link" onClick={toggleMenu}>
  ...
  const userRoute = () => {
    return (
      <ul className="navbar-nav">
        <NavLink to={`/userdetails`} className="dropdown-item">
    ...
    if (window.confirm(`Are you sure to logout?`)) {
      try {
        const res = await axios.get(
          `https://sattonjanam-be.vercel.app/api/v1/logout`
        );
        localStorage.clear();
